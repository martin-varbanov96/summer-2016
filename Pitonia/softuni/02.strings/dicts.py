...
for i, person in enumerate(people):
    for j in range(i + 1, len(people)):
        if(people[i]['gender'] is not people[j]['gender']):
            first_set = set(people[i]['interests'])
            second = set(people[j]['interests'])
            if(first_set.intersection(second) is not set()):
                print(people[i]['name'] +  " and " + people[j]['name'])